...
class AdvancedFeatureSelector:
    ...
    def _permutation_importance_fast_aggr(self, n_features=15, model=None):
        ...
        num_bins = 5
        y_bins = pd.cut(self.y, bins=num_bins, labels=False)

        X_train, X_test, y_train, y_test = train_test_split(
            self.X_scaled, 
            self.y, 
            test_size=0.20, stratify=y_bins,
            random_state=42
        )
    ...
    def _permutation_importance_cv_safe(self, n_features=15, model=None):
        ...
        for i, (train_idx, val_idx) in enumerate(skf.split(self.X_scaled, y_bins), 1):
    ...
    def _rfe_selection_cv_safe(self, n_features=15):
        ...
        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
        splits = list(skf.split(self.X_scaled, y_bins))
    ...
    def _mutual_information_cv_safe(self, n_features=15):
        ...
        for train_idx, _ in skf.split(self.X_scaled, y_bins):
