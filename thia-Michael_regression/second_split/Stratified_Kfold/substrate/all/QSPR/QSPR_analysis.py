...
class AdvancedQSPRAnalysis:
    ...
    **Pipeline Steps**:
    ...
    def run_complete_analysis(self, method='consensus', n_features=20, mode='fast_aggr', backfill=False, **kwargs):
        ...
        self.step4_hyperparameter_optimization(['RandomForestRegressor', 'XGBoost', 'GradientBoosting', 'Ridge', 'DecisionTree', 'KNN', 'SVR'])
        self.step5_train_models(['RandomForestRegressor', 'XGBoost', 'GradientBoosting', 'Ridge', 'DecisionTree', 'KNN', 'SVR'])
...
if __name__ == "__main__":
    ...
    target_features_list = [3, 4, 5, 6, 7, 8, 10] # Full spectrum scan

    for n in target_features_list:
        # --- Fast Mode (Aggressive) ---
        # 1. Strict (Only high-confidence features, may return < n)
        ...
        # 3. Strict (Only robust features validated via CV, often returns < n)
        FEATURE_SELECTION_CONFIGS.append({
            'method': 'consensus', 'n_features': n, 
    ...
    print("═" + "═"*73 + "═")
    print(" MULTI-CONFIG QSPR ANALYSIS ".center(73))
    ...
    for random_state in RANDOM_STATES:
        ...
        # ===========================
                ...
                output_filename = f'QSPR_Results_RS{random_state}_{method}_n{n_features}_{mode}_{backfill}.xlsx'
                ...
                best_row = results_df.nsmallest(1, 'Test_RMSE').iloc[0]
    ...
    summary_file = f'{logs_path}/QSPR_Summary_Multi_Config_{timestamp}.xlsx'
    ...
    if successful_runs > 0:
        ...
        best_overall_idx = successful_df['Best_Test_RMSE'].idxmin()
        ...
        print(f"  Test RMSE: {best_overall['Best_Test_RMSE']:.2f}")
        print(f"  CV RMSE: {best_overall['CV_RMSE_Mean']:.2f} ± {best_overall['CV_RMSE_Std']:.2f}")
        print(f"  Test R²: {best_overall['Best_Test_R2']:.4f}")
        print(f"  Test MAE: {best_overall['Best_Test_MAE']:.2f}")
        print(f"  CV MAE: {best_overall['CV_MAE_Mean']:.2f} ± {best_overall['CV_MAE_Std']:.2f}")
    ...
    print("═"*75)
    print(f">> Individual results: ../Results/QSPR_Results_RS*.xlsx ({successful_runs} files)")
