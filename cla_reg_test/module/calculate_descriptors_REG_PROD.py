...
class QSPRConfig:
    ...
    def __init__(self, config_name: str = "config_REG_PROD.yaml"):
...
def get_standardized_mol(smiles):
    ...
    try:
        ...
        if res == -1:
            ...
            print(f"Using Random Coordinates for {smiles}")
            cids = AllChem.EmbedMultipleConfs(mol_3d, numConfs=10, params=params)
            #res = AllChem.EmbedMolecule(mol_3d, useRandomCoords=True, maxAttempts=5000, randomSeed=42)
            if not cids:
                print(f"Standard ETKDG failed for {smiles}. Using basic Distance Geometry...")
            ...
            if cids:
                ...
                print(f"CRITICAL: 3D embedding failed completely for {smiles}")
                res = -1
...
def prepare_data(data: str, target_col: str, name: str, mode: str,
    ...
    if mode == 'models_creation':
        ...
        if not comparison_file_path.exists():
            ...
            combined_df = pd.DataFrame(data=df['SMILES_product'], columns=['SMILES_product'])
        ...
        combined_df = pd.DataFrame(data=df, columns=['SMILES_product'])
        combined_df[target_col] = targets
        if not comparison_file_path.exists():
            print("Saving SMILES comparison...")
            _save_smiles_comparison(combined_df, canonical_smiles_list, st_df, output_path, name, mode)
...
def _save_smiles_comparison(df_with_smiles, canonical_smiles_list, st_df, output_path, name, mode):
    ...
    data_smiles['Initial_SMILES'] = df_with_smiles['SMILES_product']
