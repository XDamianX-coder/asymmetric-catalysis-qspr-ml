...
class QSPRConfig:
    ...
    def __init__(self, config_name: str = "config_REG_SUB.yaml"):
...
class UniversalReactiveCCFinder:
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            # Looking for: C=C connected with C (sp3), which is bind to C=O
    ...
    def get_core_map(self, mol):
        ...
        core = {
            'beta': match[0],
            'alpha': match[1],
            'acceptor': match[2] # This is C (enone), S (sulphone), N (nitro), C (alcohol)
        }
...
def get_standardized_mol(smiles):
    ...
    try:
        ...
        if root_idx != -1:
            ...
            print(f" ROOTING WARNING: Core C=C not found for SMILES: {smiles}")
...
def prepare_data(data: str, target_col: str, name: str, mode: str,
    ...
    if mode == 'models_creation':
        ...
        if not comparison_file_path.exists():
            ...
            combined_df = pd.DataFrame(data=df['SMILES_substrate'], columns=['SMILES_substrate'])
        ...
        combined_df = pd.DataFrame(data=df, columns=['SMILES_substrate'])
        combined_df[target_col] = targets
        if not comparison_file_path.exists():
            print("Saving SMILES comparison...")
            _save_smiles_comparison(combined_df, canonical_smiles_list, st_df, output_path, name, mode)
...
def _calculate_and_clean_descriptors(df, output_path, name, mordred_features, fingerprints, 
    ...
    try:
        ...
        for smi in df['SMILES_substrate']:
        ...
        if sterimol_electronic:
            print(" >> Calculating Reactive C=C Sterimol & Electronic descriptors...")
...
def _save_smiles_comparison(df_with_smiles, canonical_smiles_list, st_df, output_path, name, mode):
    ...
    data_smiles['Initial_SMILES'] = df_with_smiles['SMILES_substrate']
