...
class AdvancedQSPRAnalysis:
    ...
    **Pipeline Steps**:
    ...
    def run_complete_analysis(self, method='consensus', n_features=20, mode='fast_aggr', backfill=False, **kwargs):
        """Execute full 7-step QSPR pipeline.
    
        Wrapper for steps 1-7. Handles multi-config runs automatically.
        
        Args:
            method (str): Feature selection method
            nfeatures (int): Target feature count
        
        Returns:
            self: Completed analysis with all results
        """
        
        
        print("\n" + "═" + "═"*73 + "═")
        print(" ADVANCED QSPR ANALYSIS - COMPLETE PIPELINE ".center(73))
        print(" With CircuS Descriptors & Comprehensive Validation ".center(73))
        print(" Including 7 ML Models with Hyperparameter Optimization ".center(73))
        print("═" + "═"*73 + "═")
        
        self.step1_load_and_prepare_data()
        self.step2_feature_selection(method=method, n_features=n_features, mode=mode, backfill=backfill, **kwargs)
        actual_features = len(self.selected_features)
        if actual_features < 2:
            print("\n" + "!"*75)
            print(f" WARNING: Only {actual_features} feature(s) selected.".center(73))
            print(" Machine Learning requires at least 2 features for valid AOA/Distance metrics.".center(73))
...
if __name__ == "__main__":
    ...
    # =====================================================================
    # MULTI-CONFIG SETUP - DEFINE YOUR CONFIGURATIONS HERE
    # =====================================================================
    desc_config = CONFIG.get('descriptors')
    circus_config = desc_config.get('circus', {})
    ...
    target_features_list = [2, 3, 4, 5, 6] 

    for n in target_features_list:
        # --- Fast Mode (Aggressive) ---
        # Best chance for small datasets as it uses the full training set (no inner CV).
        
        # Strict (High confidence only)
        FEATURE_SELECTION_CONFIGS.append({
            'method': 'consensus', 'n_features': n, 
            'search_multiplier': 3.0, 'min_methods': 2, 
            'mode': 'fast_aggr', 'backfill': False
        })
        
        # --- Safe Mode (Conservative CV) ---
        # CV reduces training size further.

        FEATURE_SELECTION_CONFIGS.append({
                'method': 'consensus', 'n_features': n, 
                'search_multiplier': 4.0, 'min_methods': 2, 
                'mode': 'cv_safe', 'backfill': False
            })
    ...
    results_summary = []
    total_runs = len(RANDOM_STATES) * len(FEATURE_SELECTION_CONFIGS)
    current_run = 0
    
    print("═" + "═"*73 + "═")
    print(" MULTI-CONFIG QSPR ANALYSIS CLASSIFICATION".center(73))
    print(f" Total runs: {total_runs} ".center(73))
    print(f" Random states: {RANDOM_STATES} ".center(73))
    print(f" Configs per RS: {len(FEATURE_SELECTION_CONFIGS)} ".center(73))
    print("═" + "═"*73 + "═\n")
    
    # Outer loop: Random states
    for random_state in RANDOM_STATES:
        
        # Inner loop: Feature selection configurations
        for config in FEATURE_SELECTION_CONFIGS:
            ...
            extra_params = {k: v for k, v in config.items()
                if k not in ['method', 'n_features', 'mode', 'backfill']}
            
            print("\n" + "▓"*75)
            print(f"RUN {current_run}/{total_runs}: RS={random_state} | Method={method} | Features={n_features}")
            if extra_params:
                print(f"Params: {extra_params}")
            print("▓"*75)
            
            try:
                # Create analysis instance with current random state
                analysis = AdvancedQSPRAnalysis()
                analysis.current_method = method
                analysis.current_nfeatures = n_features
                analysis.random_state = random_state
                analysis.current_mode = mode
                analysis.backfill = backfill
        # ===========================
                # Run complete analysis with current configuration
                ...
                if not hasattr(analysis, 'results_df') or analysis.results_df.empty:
                    print(f"  [Info] Run skipped due to insufficient features. Proceeding to next config.")
                    
                    # Log as SKIPPED instead of FAILED
                    summary = {
                        'Run': current_run,
                        'Random_State': random_state,
                        'Method': method,
                        'N_Features': n_features,
                        'Best_Algorithm': 'SKIPPED (Features < 2)',
                        'Best_Test_Acc': np.nan,
                        'Best_Test_Bal_Acc': np.nan,
                        'CV_Bal_Acc_Mean': np.nan,
                        'Best_Test_F1': np.nan,
                        'CV_Acc_Mean': np.nan,
                        'P_Value': np.nan,
                        'Significant': False,
                        'Status': 'SKIPPED', # New status
                        'Output_File': ''
                    }
                    results_summary.append(summary)
                    continue
                
                # Generate output filename with all parameters
                output_filename = f'QSPR_Classification_Results_RS{random_state}_{method}_n{n_features}_{mode}_{backfill}.xlsx'
                output_file = f'../Results/{output_filename}'
                
                # Export results
                analysis.export_results(output_file)
                
                # Extract best model results for summary
                results_df = analysis.results_df
                best_row = results_df.nlargest(1, 'Test_Bal_Acc').iloc[0]
                ...
                summary = {
                    ...
                    'Best_Test_Bal_Acc': best_row['Test_Bal_Acc'],
                    'CV_Bal_Acc_Mean': best_row['CV_Bal_Acc'],
                    'Best_Test_F1': best_row['Test_F1'],
                    'CV_Acc_Mean': best_row['CV_Acc_Mean'],
                ...
                print(f" Test Balanced Acc: {best_row['Test_Bal_Acc']:.3f}")
                print(f" Test F1-score: {best_row['Test_F1']:.2f}")
                print(f" File: {output_filename}")
                
            except Exception as e:
                print(f"  ERROR in run {current_run}")
    ...
    print("\n" + "╔" + "═"*73 + "╗")
    print("║" + " CLASSIFICATION ANALYSIS COMPLETE ".center(73) + "║")
    print("╚" + "═"*73 + "╝\n")
    ...
    summary_path = CONFIG.get('paths.logs')
    summary_file = f'{logs_path}/QSPR_Classification_Summary_Multi_Config_{timestamp}.xlsx'
    summary_df.to_excel(summary_file, index=False)
    
    # Print summary table
    print("═"*75)
    print("CLASSIFICATION ANALYSIS SUMMARY TABLE")
    print("═"*75)
    print(summary_df.to_string(index=False))
    print("═"*75)
    
    # Print statistics
    successful_runs = len([s for s in results_summary if s['Status'] == 'SUCCESS'])
    failed_runs = len([s for s in results_summary if s['Status'] == 'FAILED'])
    
    print(f"\n SUCCESSFUL RUNS: {successful_runs}/{total_runs}")
    print(f" FAILED RUNS: {failed_runs}/{total_runs}")
    
    if successful_runs > 0:
        # Find best overall result
        successful_df = summary_df[summary_df['Status'] == 'SUCCESS']
        best_overall_idx = successful_df['Best_Test_Bal_Acc'].idxmax()
        best_overall = successful_df.loc[best_overall_idx]
        
        print(f"\nBEST OVERALL RESULT:")
        print(f" Run:          {best_overall['Run']}")
        print(f" Random State: {best_overall['Random_State']}")
        print(f" Method:       {best_overall['Method']}")
        print(f" N_Features:   {best_overall['N_Features']}")
        print(f" Algorithm:    {best_overall['Best_Algorithm']}")
        print(f" Test Acc:     {best_overall['Best_Test_Acc']:.4f}")
        print(f" Test Bal Acc: {best_overall['Best_Test_Bal_Acc']:.4f}")
        print(f" CV Bal Acc:   {best_overall['CV_Bal_Acc_Mean']:.4f}")
        print(f" Test F1:      {best_overall['Best_Test_F1']:.2f}")
        print(f" File:         {best_overall['Output_File']}")

    # Print file locations
    print(f"\n" + "═"*75)
    print("OUTPUT FILES GENERATED:")
    print("═"*75)
    print(f">> Individual results: ../Results/QSPR_Classification_Results_RS*.xlsx ({successful_runs} files)")
