...
class RigorousValidator:
    ...
    def cross_validation_analysis(self, n_folds=5):
        """k-fold CV with accuracy and log-loss estimation."""
        ...
        for train_idx, val_idx in kf.split(self.X_train, self.y_train):
            ...
            if hasattr(fold_model, "predict_proba"):
                ...
                ll = log_loss(y_fold_val, y_proba)
            else:
                # Fallback: use hard predictions as probabilities
                y_proba = np.clip(
                    np.column_stack([
                        1 - (y_pred == 1).astype(float),
                        (y_pred == 1).astype(float)
                    ]),
                    1e-15, 1 - 1e-15
                )
                ll = log_loss(y_fold_val, y_proba)
        ...
        return {
            ...
            'cv_logloss_mean': np.mean(fold_logloss),
            'cv_logloss_std': np.std(fold_logloss),
        }
    ...
    def bootstrap_confidence_intervals(self, n_bootstrap=100):
        ...
        Uses predicted class probabilities for the positive class.
        """
        bootstrap_probs = []
        ...
        for boot in range(n_bootstrap):
            ...
            if hasattr(boot_model, "predict_proba"):
                y_prob_boot = boot_model.predict_proba(self.X_test)[:, 1]
            else:
                # Fallback: hard predictions to pseudo-probabilities
                y_pred_boot = boot_model.predict(self.X_test)
                y_prob_boot = y_pred_boot.astype(float)
        ...
        return {
            ...
            'median': np.percentile(bootstrap_probs, 50, axis=0),
