...
class AdvancedQSPRAnalysis:
    ...
    **Pipeline Steps**:
    ...
    def set_fixed_hyperparameters(self, fixed_params):
        """
        Build fixed pipelines for reviewer reruns without GridSearchCV.

        fixed_params:
            dict keyed by the algorithm labels used in self.step5trainmodels().
        """
        estimators = {
        ...
        print("\nUsing fixed historical hyperparameters; "
            "GridSearchCV was skipped.")


    def load_fixed_models(self, model_paths):
        """
        Load historical final pipelines and skip GridSearchCV.
        """
        import joblib
        ...
        print("\nLoaded historical frozen pipelines; "
            "GridSearchCV was skipped.")
    # Review 1
    ...
    def export_reviewer_validation_outputs(self, result, tag):
        outdir = Path(CONFIG.get("paths.results", "Results")) / "ReviewerValidation"
        ...
        shuffle_rmse = result["target_shuffling_rmse"]

        if len(shuffle_rmse) == 0:
            raise ValueError(
                f"Target-shuffling distribution is empty for {tag}."
            )

        shuffle_df = pd.DataFrame({
            "shuffle_id": np.arange(1, len(shuffle_rmse) + 1),
            "RMSE": shuffle_rmse,
        })
        ...
        ts = result["target_shuffling"]

        summary_df = pd.DataFrame([{
            "observed_RMSE": result["test_rmse"],
            "observed_MAE": result["test_mae"],
            "observed_R2": result["test_r2"],
            "null_RMSE_mean": ts.get("null_rmse_mean"),
            "null_RMSE_std": ts.get("null_rmse_std"),
            "null_RMSE_min": ts.get("null_rmse_min"),
            "null_RMSE_max": ts.get("null_rmse_max"),
            "empirical_one_sided_permutation_p": ts.get("p_value"),
            "z_score": ts.get("z_score"),
            "n_shuffles": len(shuffle_rmse),
        }])
        ...
        raw_bootstrap_df = pd.DataFrame(
            boot["bootstrap_predictions"],
            columns=[
                f"test_{idx}"
                for idx in result["X_test"].index.to_numpy()
            ],
        )
        ...
        print(
            f"[Reviewer validation] Saved CV, target-shuffling, "
            f"and bootstrap outputs for {tag}"
        )
    #Review round 1
    ...
    def run_complete_analysis(self, 
        ...
        if preselected_features is not None and len(preselected_features) > 0:
            ...
            # Historical model used only as the source of the canonical feature order.
            reference_model_name = "RandomForestRegressor"

            if (
                not isinstance(getattr(self, "hyperopt_results", None), dict)
                or reference_model_name not in self.hyperopt_results
            ):
                available_entries = (
                    list(self.hyperopt_results.keys())
                    if isinstance(getattr(self, "hyperopt_results", None), dict)
                    else []
                )
                raise RuntimeError(
                    f"Frozen reference model '{reference_model_name}' was not found "
                    f"in self.hyperopt_results. Available entries: {available_entries}"
                )

            reference_entry = self.hyperopt_results[reference_model_name]

            # `load_fixed_models()` may save the estimator directly or as an entry
            # such as {"model": estimator, "best_params": {...}}.
            if hasattr(reference_entry, "feature_names_in_"):
                reference_model = reference_entry

            elif isinstance(reference_entry, dict):
                reference_model = next(
                    (
                        reference_entry[key]
                        for key in ("best_model", "model", "estimator", "pipeline")
                        if key in reference_entry
                        and hasattr(reference_entry[key], "feature_names_in_")
                    ),
                    None,
                )
                ...
                if reference_model is None:
                    raise TypeError(
                        f"Could not find a fitted estimator in "
                        f"self.hyperopt_results['{reference_model_name}']. "
                        f"Available keys: {list(reference_entry.keys())}"
                    )

            else:
                raise TypeError(
                    f"Unexpected object type for "
                    f"self.hyperopt_results['{reference_model_name}']: "
                    f"{type(reference_entry).__name__}"
                )

            # Exact historical column sequence from the frozen fitted model.
            model_feature_order = list(reference_model.feature_names_in_)

            # Current descriptor availability: fail loudly rather than silently
            # dropping a feature expected by a fitted frozen model.
            ...
            if missing_from_current_data:
                raise KeyError(
                    f"Current data are missing features expected by frozen "
                    f"'{reference_model_name}': {missing_from_current_data}"
                )

            # The handwritten list may differ in ORDER, but must contain the
            # identical SET of features.
            if set(preselected_features) != set(model_feature_order):
                manual_only = [
                    feature for feature in preselected_features
                    if feature not in model_feature_order
                ]
                model_only = [
                    feature for feature in model_feature_order
                    if feature not in preselected_features
                ]

                raise ValueError(
                    "PRESELECTED_FEATURES and the frozen reference model use "
                    "different feature sets.\n"
                    f"Only in PRESELECTED_FEATURES: {manual_only}\n"
                    f"Only in frozen model: {model_only}\n"
                    f"Manual features ({len(preselected_features)}): "
                    f"{preselected_features}\n"
                    f"Frozen features ({len(model_feature_order)}): "
                    f"{model_feature_order}"
                )

            # The frozen model order is authoritative.
            valid_features = model_feature_order

            self.selected_features = pd.DataFrame(
                {"feature": valid_features}
            )

            # Create the only model matrices intended for downstream Steps 3, 5, 6,
            # and 7. `.loc[:, valid_features]` enforces the saved column order.
            self.X_train_model = self.train_data.loc[:, valid_features].copy()
            self.X_test_model = self.test_data.loc[:, valid_features].copy()

            self.method_name = "preselected"
            self.current_method = "preselected"
            self.current_nfeatures = len(valid_features)
            self.current_mode = "preselected"
            self.backfill = False

            print(f"→ Using {len(valid_features)} pre-selected features (Step 2 skipped).")
            print(f"  Reference frozen model: {reference_model_name}")
            print(
                "  Manual order equals frozen order: "
                f"{list(preselected_features) == valid_features}"
            )
            print(f"  Feature order: {valid_features}")
            print(f"  Training model matrix: {self.X_train_model.shape}")
            print(f"  Test model matrix:     {self.X_test_model.shape}")

            # Correlations should be computed only on the training partition.
            print("\n" + "=" * 75)
            self.correlation_df = print_feature_correlations(
                self.train_data,
        ...
        if not skip_hyperparameter_optimization:
            self.step4hyperparameteroptimization()
...
if __name__ == "__main__":
    ...
    "RandomForestRegressor": (
        ...
        "RS42_consensus_n8_cv_safe_False_RandomForestRegressor_model.pkl"
    ),
    "XGBoost": (
        "../Results/Trained_Models/"
        "RS42_consensus_n8_cv_safe_False_XGBoost_model.pkl"
    ),
    "GradientBoosting": (
        "../Results/Trained_Models/"
        "RS42_consensus_n8_cv_safe_False_GradientBoosting_model.pkl"
    ),
    "Ridge": (
        "../Results/Trained_Models/"
        "RS42_consensus_n8_cv_safe_False_Ridge_model.pkl"
    ),
    "DecisionTree": (
        "../Results/Trained_Models/"
        "RS42_consensus_n8_cv_safe_False_DecisionTree_model.pkl"
    ),
    "KNN": (
        "../Results/Trained_Models/"
        "RS42_consensus_n8_cv_safe_False_KNN_model.pkl"
    ),
    "SVR": (
        "../Results/Trained_Models/"
        "RS42_consensus_n8_cv_safe_False_SVR_model.pkl"
    ),
    }
    ...
    try:
        ...
        #Review 1
        print("\nLOADED ATTRIBUTES:")
        print(sorted(vars(analysis).keys()))

        for attr_name in ("fixed_models", "models", "loaded_models", "frozen_models", "hyperopt_results"):
            value = getattr(analysis, attr_name, None)

            if isinstance(value, dict):
                print(f"\n{attr_name}: dict with keys:")
                print(list(value.keys()))
            else:
                print(f"\n{attr_name}: {type(value).__name__}")
    # Review 1
