...
from sklearn.feature_selection import RFECV, f_regression, RFE, mutual_info_regression
...
class AdvancedFeatureSelector:
    ...
    def _permutation_importance_fast_aggr(self, n_features=15, model=None):
        ...
        Uses a single train/test split to approximate out-of-sample 
        ...
        num_bins = 4
        y_bins = pd.qcut(self.y, q=num_bins, labels=False)
    ...
    def _permutation_importance_cv_safe(self, n_features=15, model=None):
        ...
        for i, (train_idx, val_idx) in enumerate(kf.split(self.X_scaled, y_bins), 1):
    ...
    def _rfe_selection_cv_safe(self, n_features=15):
        ...
        splits = list(cv.split(self.X_scaled, y_bins))
    ...
    def _mutual_information_cv_safe(self, n_features=15):
        ...
        for train_idx, _ in kf.split(self.X_scaled, y_bins):
...
class BaselineModels:
    ...
    - baseline_dummy_mean(): Absolute performance floor (predicts mean of y_train)
    - baseline_target_shuffling(): Null hypothesis test with optimized estimator
      (100 permutations) → strong randomization test
    """
    
    @staticmethod
    def baseline_dummy_mean(y_train, y_test):
        """Dummy: predict mean"""
        mean_pred = np.full_like(y_test, np.mean(y_train), dtype=float)
    ...
    @staticmethod
    def baseline_target_shuffling(X_train, X_test, y_train, y_test, estimator, n_shuffles=100):
        """
        ...
        Args:
            ...
            estimator: Fitted estimator (Pipeline or model) from GridSearchCV
            n_shuffles: Number of permutations (default: 100)
        
        Returns:
            tuple: (mean_null_rmse, std_null_rmse, list_of_null_rmse)
        """
        null_rmse_list = []
        ###null_mae_list = []
        os.environ['PYTHONWARNINGS'] = 'ignore'
        ...
        for shuffle_iter in range(n_shuffles):
            ...
            # Clone the optimized estimator (preserves hyperparameters!)
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    # Fallback for edge cases (extremely small data or convergence issues)
                    continue
            
            y_pred = model_shuffled.predict(X_test)
            #mae = mean_absolute_error(y_test, y_pred)
            rmse = math.sqrt(mean_squared_error(y_test, y_pred))
            null_rmse_list.append(rmse)
        
        # Safety check (if all iterations failed)
        if not null_rmse_list:
            return 0.0, 0.0, []
        
        return np.mean(null_rmse_list), np.std(null_rmse_list), null_rmse_list
...
class HyperparameterOptimizer:
    ...
    Comprehensive parameter grids with nested 5-fold CV (scoring='neg_root_mean_squared_error' or 'r2' or 'neg_mean_absolute_error').
    ...
    def _optimize_pipeline(X_train, y_train, estimator, param_grid_raw, cv=5):
        ...
        num_bins = 4
        y_bins = pd.qcut(y_train, q=num_bins, labels=False)
        kf = StratifiedKFold(n_splits=cv, shuffle=True, random_state=42)
        splits = list(kf.split(X_train, y_bins))
        # Build Pipeline
        pipeline = Pipeline([
    ...
    @staticmethod
    def optimize_ridge_regression(X_train, y_train, cv=5):
        """GridSearchCV optimization for Ridge"""
        return HyperparameterOptimizer._optimize_pipeline(
            X_train, y_train, 
            Ridge(),
            HyperparametersConfig.RIDGE_REGRESSION, cv=cv
        )
    ...
    def optimize_knn(X_train, y_train, cv=5):
        """GridSearchCV optimization for KNN"""
        return HyperparameterOptimizer._optimize_pipeline(
            X_train, y_train, 
            KNeighborsRegressor(),
    ...
    @staticmethod
    def optimize_svr(X_train, y_train, cv=5):
...
def prepare_model(X_train, X_test, y_train, y_test,
    ...
    # Statistical test
    ts_stats = RigorousValidator.statistical_test(test_rmse, null_dist)
    ...
    return {
        ...
        'target_shuffling_rmse': null_dist,
...
class RigorousValidator:
    ...
    def cross_validation_analysis(self, n_folds=5):
        """5-fold CV with uncertainty quantification"""
        num_bins = 4
        y_bins = pd.qcut(self.y_train, q=num_bins, labels=False)
        ...
        for train_idx, val_idx in kf.split(self.X_train, y_bins):
            ...
            fold_rmse_score = math.sqrt(mean_squared_error(y_fold_val, y_pred))
            fold_mae_score = mean_absolute_error(y_fold_val, y_pred)
            
            fold_mae.append(fold_mae_score)
            cv_scores.append(fold_r2)
            fold_rmse.append(fold_rmse_score)
        
        return {

            # Full fold-wise results for export
            'fold': np.arange(1, n_folds + 1),
            'r2': np.asarray(cv_scores, dtype=float),
            'rmse': np.asarray(fold_rmse, dtype=float),
            'mae': np.asarray(fold_mae, dtype=float),

            'cv_r2': np.array(cv_scores),
    ...
    def bootstrap_confidence_intervals(self, n_bootstrap=100):
        ...
        for boot in range(n_bootstrap):
            ...
            y_pred_boot = boot_model.predict(self.X_test)
            bootstrap_preds.append(y_pred_boot)
        
        bootstrap_preds = np.array(bootstrap_preds)
        
        return {
            # Existing summary fields retained for compatibility
            ...
            'std_dev': np.std(bootstrap_preds, axis=0),

            # Full per-test-observation reporting metadata
            'test_index': self.X_test.index.to_numpy(),
            'observed_target': np.asarray(self.y_test, dtype=float),
            ...
            # Raw predictions: shape = (n_bootstrap, n_test_observations)
    ...
    @staticmethod
    def statistical_test(our_rmse, null_rmse_dist):
        ...
        """
        null_mean = np.mean(null_rmse_dist)
        null_std = np.std(null_rmse_dist)
        n_null = len(null_rmse_dist)
        
        # 1. Z-score (Distance in standard deviations)
        # How many sigmas is our model away from noise mean?
        # (null_mean - our_rmse) we want the result to be positive if model is better (smaller issue)
        z_score = (null_mean - our_rmse) / null_std if null_std > 0 else 0
        
        # 2. Empirical p-value (Definition of Permutation Test)
        # Proportion of random models that were better (lower RMSE) than ours.
        # We add 1 to the numerator and denominator to avoid p = 0 (add‑one / Laplace smoothing for a conservative p‑value estimate).
        n_better_random = np.sum(np.array(null_rmse_dist) <= our_rmse)
        p_value = (n_better_random + 1) / (n_null + 1)
        
        # 3. Decision Rules
        better_than_null = our_rmse < null_mean
        
        # Criteria:
        # - Significant: p < 0.05
        # - Truly Significant: Better by > 2 sigmas (Z > 2) OR p < 0.01
        
        significant = p_value < 0.05
        
        # Rule: our_rmse < null_mean - 2*std
        ...
        return {
            ...
            'null_rmse_mean': null_mean,
            'null_rmse_std': null_std,
            'null_rmse_min': np.min(null_rmse_dist),
            'null_rmse_max': np.max(null_rmse_dist),
            'threshold_good': null_mean - 2 * null_std,
            'our_rmse': our_rmse
        }
